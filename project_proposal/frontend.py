...
# Affichage du statut
count = len(st.session_state.observations)
st.markdown(f"**Observations collectées : {count} / 6**")
...
# --- 4. Logique du Bouton d'Ajout (Collecte des 6) ---
if count < 6:
    # Le bouton est actif tant qu'on a moins de 6 observations
    if st.button("Ajouter cette observation à la séquence"):
        # On ajoute l'observation à la liste en mémoire
        st.session_state.observations.append(data.copy())
        st.success(f"Observation ajoutée avec succès. ({count + 1}/6)")
        
        # Le rerun permet d'actualiser immédiatement le compteur et le DataFrame.
        st.rerun()
else:
    st.success("Séquence de 6 jours complète. Prêt pour la prédiction !")

# Affichage de la séquence collectée
if st.session_state.observations:
    st.subheader("Séquence des 6 Observations Collectées")
    df_sequence = pd.DataFrame(st.session_state.observations)
...
if count == 6:
    ...
    try:
        ...
        if st.button("Envoyer"):
            ...
            # Le payload de l'API doit contenir la séquence complète de 6 jours
            # La clé 'observations' contiendra la liste des 6 dictionnaires de st.session_state

            # l'ensemble de mes données, est dans payload_api
            payload_api = {"observations": st.session_state.observations}
            st.json(payload_api)
            response  = requests.post(url, json = payload_api) 
